...
1. [x] Migración `V112` — agrega `member_id` (FK nullable a `family_members`) a `journal_entries`; agrega `perceived_event_key` (`VARCHAR(64)` nullable, indexado) y `visibility` (string, default `PRIVATE`) a `journal_entries` y `critical_days`.
