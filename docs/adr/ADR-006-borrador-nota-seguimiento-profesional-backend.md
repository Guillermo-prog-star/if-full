...
1. [ ] Migración `V107` — tabla `professional_follow_up_drafts` (columnas de Decisión 1) + columna nullable `follow_up_draft_id` en `support_professional_notes`.
2. [ ] Entidad `ProfessionalFollowUpDraft` + enum `DraftStatus {GENERATED, VOIDED}` + enum `GeneratorType {RULE_BASED_TEMPLATE}` (`com.integrityfamily.support.domain`).
3. [ ] `ProfessionalFollowUpDraftService` — puerto de `generateAiSummaryText()`/`recomendacionesPlan` desde `professional-dashboard.component.ts:284-343`, versionado como `professional-follow-up-v1.0`; marca `VOIDED` el borrador anterior del mismo `assignmentId` al generar uno nuevo.
4. [ ] `POST /api/families/{familyId}/support/follow-up-drafts` en `SupportNetworkController`, mismo `@PreAuthorize` que `/notes`.
5. [ ] `AuditEventType.PROFESSIONAL_DRAFT_GENERATED` / `PROFESSIONAL_DRAFT_USED_AS_NOTE`, invocados vía `AuditService.registerSystemEvent`.
6. [ ] Frontend: eliminar `generateAiSummaryText()`; `professional-dashboard.component.ts` llama al nuevo endpoint; leyenda de disclaimer construida desde `warnings` de la respuesta en vez de hardcodeada en `professional-dashboard.component.html:224-225`.
7. [ ] Tests unitarios de `ProfessionalFollowUpDraftService` (paridad con las reglas actuales) + test de integración del endpoint + `FamilyLifecycleIntegrationTest` si aplica.
